...
socket_connect($socket, MIFAREIP, MIFAREPORT);
/*
...
$out  = array(0x00);
      ...
      try{
         ...
         /*
         $outPan = socket_read($socket, 1024);
         $res = strlen($outPan);
         while($res){
            array_push($out, str_split($outPan, 1));
            $outPan = socket_read($socket, 1024);
            $res = strlen($outPan);
         }*/
      }catch(Exception $e){
